        ...
        return{
            ...
            link: function (a, g, c) {
                var e = c.treeModel,
                    ...
                    k = '<ul><li data-ng-repeat="node in ' + e + '" ui-sref-active="active"><i class="collapsed" data-ng-show="node.' + d + '.length && node.collapsed" data-ng-click="selectNodeHead(node, $event)"></i><i class="expanded" data-ng-show="node.' + d + '.length && !node.collapsed" data-ng-click="selectNodeHead(node, $event)"></i><a ui-sref="reports.details({levelId:node.levelId})" class="collapsed1" data-ng-show="node.' + d + '.length && node.collapsed" data-ng-click="selectNodeHead(node, $event)" >{{node.' + h + '}}</a><a ui-sref="reports.details({levelId:node.levelId})" class="expanded1" data-ng-show="node.' + d + '.length && !node.collapsed">{{node.' + h + '}}</a><a ui-sref="reports.details({levelId:node.levelId})" data-ng-hide="node.' +
